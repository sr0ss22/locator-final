...
const AdminToolsPage: React.FC = () => {
  ...
  const [updateStateLoading, setUpdateStateLoading] = useState(false); // New state for the new tool
  ...
  // New handler for the state update function
  const handleUpdateStateCodes = async () => {
    setUpdateStateLoading(true);
    const loadingToastId = toast.loading('Starting state code update process...');

    try {
      const { data, error } = await supabase.functions.invoke('update-zip-state');
      ...
      console.error('Error invoking update-zip-state function:', error);
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              This tool runs a script to update the `state_province` for all US ZIP codes in the database using an internal data file. This is a one-time operation to fix data inconsistencies.
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button onClick={handleUpdateStateCodes} disabled={updateStateLoading}>
