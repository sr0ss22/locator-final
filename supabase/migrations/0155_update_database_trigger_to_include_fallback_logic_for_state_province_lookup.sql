...
BEGIN
    ...
    -- 2. For all codes, check the main geometries table.
    SELECT state_province INTO _state_province
    FROM public.zip_code_geometries
    WHERE zip_code = NEW.zip_code
    LIMIT 1;

    IF _state_province IS NOT NULL AND _state_province <> 'Unknown' AND _state_province <> '' THEN
        NEW.state_province := _state_province;
        RETURN NEW;
    END IF;

    -- 3. Fallback for US ZIP codes using prefix logic if the above lookups fail.
