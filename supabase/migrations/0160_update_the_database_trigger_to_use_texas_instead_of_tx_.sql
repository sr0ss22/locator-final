...
AS $function$
DECLARE
    _correct_state_province TEXT := NULL;
BEGIN
    -- This trigger will attempt to correct the state/province for US ZIP codes.
    -- It runs on every insert or update, forcefully overwriting incorrect data.

    -- We only apply this logic to what looks like a US ZIP code.
    IF NEW.zip_code ~ '^[0-9]{5}$' THEN
        
        -- This is the same logic used in the manual "Update States" admin tool.
        SELECT
            CASE
                -- Northeast
                ...
                WHEN left(NEW.zip_code, 3) BETWEEN '750' AND '799' OR left(NEW.zip_code, 3) = '885' THEN 'Texas'
                -- West
                ...
                ELSE NULL
            END
        INTO _correct_state_province;

        -- If the fallback logic found a valid state, forcefully overwrite the value on the new row.
        IF _correct_state_province IS NOT NULL THEN
            NEW.state_province := _correct_state_province;
        END IF;
    END IF;
