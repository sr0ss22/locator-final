...
BEGIN
    -- Always try to find the correct state/province, overwriting any incoming value if a better one is found.
    ...
    -- 3. If still not found (or if the found value is invalid), use the fallback for US ZIP codes.
    IF (_state_province IS NULL OR trim(COALESCE(_state_province, '')) = '' OR _state_province = 'Unknown') AND NEW.zip_code ~ '^[0-9]{5}$' THEN
        SELECT
            CASE
                -- Northeast
    ...
    -- If we found a valid state/province from any method, overwrite the incoming value.
