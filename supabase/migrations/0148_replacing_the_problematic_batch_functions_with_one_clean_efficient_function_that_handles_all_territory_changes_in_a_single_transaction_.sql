-- 1. Ensure any old, conflicting versions of the function are removed
...
-- This avoids all type-conversion errors between the browser and the database.
...
BEGIN
  ...
  -- We set a session variable that the audit trigger checks. 
  -- This tells the trigger to skip the heavy row-by-row logging.
  PERFORM set_config('my_app.bulk_op_in_progress', 'true', true);

  -- Safely identify the acting user
  BEGIN
  ...
  -- A. MASS DELETION (Direct set operation)
  IF p_removed_zips IS NOT NULL AND jsonb_array_length(p_removed_zips) > 0 THEN
    ...
    AND zip_code IN (
      SELECT value->>'zip_code' FROM jsonb_array_elements(p_removed_zips)
      UNION
      SELECT value::text FROM jsonb_array_elements(p_removed_zips) WHERE jsonb_typeof(value) = 'string'
    );
  END IF;
  ...
  -- D. RECORD SINGLE AUDIT SUMMARY
  _summary := 'Bulk update: Removed ' || COALESCE(jsonb_array_length(p_removed_zips), 0) || 
