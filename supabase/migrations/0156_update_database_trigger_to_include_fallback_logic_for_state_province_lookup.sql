...
BEGIN
    -- Only run the logic if the state is missing or invalid ('', 'Unknown')
    IF NEW.state_province IS NULL OR NEW.state_province = '' OR NEW.state_province = 'Unknown' THEN

        -- First, try to look up from our primary data tables
        IF NEW.zip_code ~ '^[A-Z]' THEN -- It's a Canadian code
        ...
        -- If it's not a Canadian code or the lookup failed, try the main geometries table
        ...
        IF _state_province = 'Unknown' OR _state_province = '' THEN
            _state_province := NULL;
        END IF;

        -- If we still don't have a state, and it's a US zip, use the prefix fallback logic
        ...
        -- If we found a state/province from any method, update the row
