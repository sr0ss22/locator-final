...
DECLARE
  ...
  _summary TEXT;
BEGIN
  -- A. Suspend auditing to prevent trigger overhead / timeouts
  ...
  -- C. ULTRA-FAST DELETE (The primary bottleneck)
  -- We extract the zip codes into a flat text array first for the fastest possible lookup
  IF p_removed_zips IS NOT NULL AND jsonb_array_length(p_removed_zips) > 0 THEN
    ...
    AND zip_code IN (
      SELECT COALESCE(v->>'zip_code', v->>'zipCode', v#>>'{}')
      FROM jsonb_array_elements(p_removed_zips) v
    );
  END IF;
  ...
  -- F. Log one single entry for the whole chunk
  _summary := format('Batch chunk processed for %s', p_installer_id);
  INSERT INTO public.territory_audit_log (installer_id, zip_code, change_type, assigned_by, summary)
  VALUES (p_installer_id, 'N/A', 'bulk_chunk', _user_id, _summary);
