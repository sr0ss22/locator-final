...
BEGIN
    ...
    WHERE 
        ...
        -- First, use the index to find points within a bounding box (fast)
        AND cpc.geog && ST_Expand(center_point, radius_meters) 
        -- Then, run the more precise distance check on the smaller result set (also fast)
