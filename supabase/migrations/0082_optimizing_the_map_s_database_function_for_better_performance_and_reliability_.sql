-- Analyze the table one last time to ensure the query planner has up-to-date statistics.
ANALYZE public.canadian_postal_codes;

-- Update the function that COUNTS points to use the index-based pre-filter.
...
BEGIN
    ...
    WHERE cpc.geog IS NOT NULL 
      AND cpc.geog && ST_Expand(center_point, radius_meters) -- Bounding box pre-filter for performance
      AND ST_DWithin(cpc.geog, center_point, radius_meters); -- Accurate distance check

    RETURN point_count;
END;
$function$;

-- Update the main function that FETCHES map data to use the same optimization.
